...
- **It doesn't replace testing.** AI assistants should write tests for any feature they implement, not just stories. Continue using `pnpm test` for unit tests, `pnpm test:ct` (Playwright) for interaction tests, and the a11y addon for accessibility checks. The MCP tools help the AI write *better* code and stories — they don't run tests.
