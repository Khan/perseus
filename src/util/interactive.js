    ...
    createSorter: function() {
                    ...
                    if (event.type === "vmousedown" && (event.which === 1 || event.which === 0)) {
                        ...
                        $(document).bind("vmousemove.tile vmouseup.tile", function(event) {
                            ...
                            if (event.type === "vmousemove") {
                                ...
                                const leftEdge = list.offset().left;
                                const midWidth = $(tile).offset().left - leftEdge;
                                let index = 0;
                                let sumWidth = 0;
                                ...
                                if (index !== tileIndex) {
                                    ...
                                    if (index === 0) {
                                        ...
                                        placeholder.detach();
                                        $(tile).detach();
                                        const preceeding = list.find("li")[index - 1];
        ...
        sorter.setContent = function(content) {
            ...
            $.each(content, function(n, sortKey) {
                const tile = list.find("li .sort-key").filter(function() {
...
_.extend(GraphUtils.Graphie.prototype, {
    ...
    addMouseLayer: function(options) {
        ...
        if (options.onClick || options.onMouseDown || options.onMouseMove ||
            ...
            const canvasClickTarget = graph.mouselayer.rect(
    ...
    getMousePx: function(event) {
        ...
        const mouseX = event.pageX - $(graphie.raphael.
            canvas.parentNode).offset().left;
        const mouseY = event.pageY - $(graphie.raphael.
    ...
    drawArcs: function(point1, vertex, point3, numArcs) {
        let startAngle = GraphUtils.findAngle(point1, vertex);
        let endAngle = GraphUtils.findAngle(point3, vertex);
    ...
    addMovablePoint: function(options) {
        const movablePoint = $.extend(true, {
            graph: this,
            coord: [0, 0],
            snapX: 0,
            snapY: 0,
    ...
    addInteractiveFn: function(fn, options) {
        ...
        const mouseAreaWidth = 30;
        const points = [];
        const step = (options.range[1] - options.range[0]) / 100;

        const addScaledPoint = function(x, y) {
            if (options.swapAxes) {
        ...
        for (let x = options.range[0]; x <= options.range[1]; x += step) {
            const ddx = (fn(x - 0.001) - fn(x + 0.001)) / 0.002;
            let x1 = x;
            let y1 = fn(x) + (mouseAreaWidth / (2 * graph.scale[1]));

            if (ddx !== 0) {
                const normalslope = (-1 / (ddx * (graph.scale[1] / graph.scale[0]))) / (graph.scale[1] / graph.scale[0]);
                if (ddx < 0) {
        ...
        for (let x = options.range[1]; x >= options.range[0]; x -= step) {
            const ddx = (fn(x - 0.001) - fn(x + 0.001)) / 0.002;
            let x1 = x;
            let y1 = fn(x) - (mouseAreaWidth / (2 * graph.scale[1]));
        ...
        $(interactiveFn.mouseTarget[0]).bind("vmouseover vmouseout vmousemove", function(event) {
            event.preventDefault();
            let mouseX = event.pageX - $(graph.raphael.canvas.parentNode).offset().left;
            let mouseY = event.pageY - $(graph.raphael.canvas.parentNode).offset().top;
            // can't go beyond 10 pixels from the edge
            ...
            const findDistance = function(coordX, coordY) {
                let closestX = 0;
                let minDist = Math.sqrt((coordX) * (coordX) + (coordY) * (coordY));
                for (let x = options.range[0]; x < options.range[1]; x += ((options.range[1] - options.range[0]) / graph.xpixels)) {
            ...
            if (options.swapAxes) {
                const closestX = findDistance(coordY, coordX);
                coordX = fn(closestX);
                coordY = closestX;
            } else {
                const closestX = findDistance(coordX, coordY);
                coordX = closestX;
                coordY = fn(closestX);
            }
    ...
    addMovableLineSegment: function(options) {
        ...
        const normalColor = (lineSegment.fixed) ? KhanColors.DYNAMIC
                                              : KhanColors.INTERACTIVE;
        ...
        lineSegment.transform = function(syncToPoints) {
            ...
            const isClockwise = (this.coordA[0] < this.coordZ[0]) ||
        ...
        if (!lineSegment.fixed && !lineSegment.constraints.fixed) {
            ...
            $mouseTarget.bind("vmousedown vmouseover vmouseout", function(event) {
                if (event.type === "vmouseover") {
                    ...
                    event.preventDefault();
                    let coordX = (event.pageX - $(graph.raphael.canvas.parentNode).offset().left) / graph.scale[0] + graph.range[0][0];
                    let coordY = graph.range[1][1] - (event.pageY - $(graph.raphael.canvas.parentNode).offset().top) / graph.scale[1];
    ...
    addMovablePolygon: function(options) {
        ...
        const normalColor = (polygon.fixed) ? KhanColors.DYNAMIC
                                          : KhanColors.INTERACTIVE;
        ...
        if (!polygon.fixed) {
            ...
            $(polygon.mouseTarget[0]).bind("vmousedown vmouseover vmouseout", function(event) {
                if (event.type === "vmouseover") {
                    ...
                    let startX = (event.pageX - $(graphie.raphael.canvas.parentNode).offset().left) / graphie.scale[0] + graphie.range[0][0];
                    let startY = graphie.range[1][1] - (event.pageY - $(graphie.raphael.canvas.parentNode).offset().top) / graphie.scale[1];
                    if (polygon.snapX > 0) {
                    ...
                    $(document).bind("vmousemove.polygon vmouseup.polygon", function(event) {
                        ...
                        let mouseX = event.pageX - $(graphie.raphael.canvas.parentNode).offset().left;
                        let mouseY = event.pageY - $(graphie.raphael.canvas.parentNode).offset().top;
    ...
    addArrowWidget: function(options) {
        const arrowWidget = $.extend({
            graph: this,
            direction: "up",
            coord: [0, 0],
            onClick: function() {},
        }, options);
        const graph = arrowWidget.graph;
        ...
        if (arrowWidget.direction === "up") {
            arrowWidget.visibleShape = graph.path([
                    ...
                    [arrowWidget.coord[0] + 4 / graph.scale[0], arrowWidget.coord[1] - 4 / graph.scale[1]],
                    [arrowWidget.coord[0], arrowWidget.coord[1] - 4 / graph.scale[1]],
            ], { stroke: "", fill: KhanColors.INTERACTIVE });
        ...
        _.defer(function() {
            arrowWidget.visibleShape.attr({stroke: "", fill: KhanColors.INTERACTIVE});
        });
        ...
        $(arrowWidget.mouseTarget[0]).bind("vmousedown vmouseover vmouseout", function(event) {
            if (event.type === "vmouseover") {
                arrowWidget.visibleShape.animate({ scale: 2, fill: KhanColors.INTERACTING }, 20);
            } else if (event.type === "vmouseout") {
                arrowWidget.visibleShape.animate({ scale: 1, fill: KhanColors.INTERACTING }, 20);
            } else if (event.type === "vmousedown" && (event.which === 1 || event.which === 0)) {
    ...
    addRectGraph: function(options) {
        ...
        rect = $.extend({
            ...
            getWidth: function() {
                if (rect.initialized()) {
                    const x0 = rect.points[1].coord[0];
                    const x1 = rect.points[2].coord[0];
                    return x1 - x0;
                }
                return rect.width;
            },
            getHeight: function() {
                if (rect.initialized()) {
                    const y0 = rect.points[0].coord[1];
                    const y1 = rect.points[1].coord[1];
                    return y1 - y0;
                }
                return rect.height;
            },
            ...
            getRaphaelParamsArr: function() {
                ...
                const x = rect.getX();
                const y = rect.getY();
                const point = graphie.scalePoint([x, y + height]);
                const dims = graphie.scaleVector([width, height]);
                return point.concat(dims);
            },
            getRaphaelParams: function() {
                const arr = rect.getRaphaelParamsArr();
                return {
                    x: arr[0],
                    y: arr[1],
                    width: arr[2],
        ...
        const coords = [[rect.x, rect.y], [rect.x, rect.y2()], [rect.x2(), rect.y2()], [rect.x2(), rect.y]];
        const sames = [[1, 3], [0, 2], [3, 1], [2, 0]];
        const moveLimits = [[1, 1], [1, 0], [0, 0], [0, 1]];
        ...
        function moveIsInBounds(index, newX, newY) {
            const xlims = rect.getXLims();
            const ylims = rect.getYLims();

            const i = moveLimits[index];

            const xInBounds = coordInBounds(xlims[i[0]], newX, i[0] === 1);
            const yInBounds = coordInBounds(ylims[i[1]], newY, i[1] === 1);

            return xInBounds && yInBounds;
        }

        _.times(4, function(i) {
            const sameX = sames[i][0];
            const sameY = sames[i][1];
            const coord = coords[i];

            const point = graphie.addMovablePoint({
                graph: graphie,
                coord: coord,
        ...
        rect.moveEdge = function(dx, dy, edgeIndex) {
            const a = rect.edges[edgeIndex].pointA;
            const z = rect.edges[edgeIndex].pointZ;
        ...
        _.times(4, function(i) {
            const pointA = rect.points[i];
            const pointZ = rect.points[(i + 1) % 4];
            const constrainX = (i % 2);
            const constrainY = ((i + 1) % 2);

            const edge = graphie.addMovableLineSegment({
                graph: graphie,
                pointA: pointA,
                pointZ: pointZ,
        ...
        const elems = [rect.fillArea, rect.mouseTarget];
        ...
        function constrainTranslation(dx, dy) {
            const xC = rect.constraints.constrainX;
            const xLT = rect.getX() + dx < rect.constraints.xmin;
            const xGT = rect.getX2() + dx > rect.constraints.xmax;
            const yC = rect.constraints.constrainY;
            const yLT = rect.getY() + dy < rect.constraints.ymin;
            const yGT = rect.getY2() + dy > rect.constraints.ymax;
        ...
        rect.translate = function(dx, dy) {
            ...
            const d = constrainTranslation(dx, dy);
            dx = d[0];
            dy = d[1];

            _.each(rect.points, function(point, i) {
                const x = point.coord[0] + dx;
                const y = point.coord[1] + dy;
                // move points
                point.setCoord([x, y]);
                // move edges
        ...
        rect.moveTo = function(x, y) {
            const dx = x - rect.getX();
            const dy = y - rect.getY();
        ...
        rect.snap = function() {
            ...
            let dy;
            _.each(rect.points, function(point, i) {
                const x0 = point.coord[0];
                const y0 = point.coord[1];
                let x1 = x0;
                let y1 = y0;

                if (rect.snapX) {
                    x1 = KhanMath.roundToNearest(rect.snapX, x0);
                }
                if (rect.snapY) {
                    y1 = KhanMath.roundToNearest(rect.snapY, y0);
                }
        ...
        const bindTranslation = function() {
                ...
                "vmouseover vmouseout vmousedown", function(event) {
                    if (event.type === "vmouseover") {
                        ...
                        $(document).on("vmousemove vmouseup", function(event) {
                            ...
                            if (event.type === "vmousemove") {
                                const currCoord = graphie.getMouseCoord(event);

                                if (rect.prevCoord && rect.prevCoord.length === 2) {
                                    const diff = GraphUtils.coordDiff(rect.prevCoord, currCoord);
    ...
    interactiveEllipse: function(options) {
        ...
        ellipse.showPoint = function(event) {
            ...
            const dx = ellipse.yRadius * (ellipse.center[0] - coord[0]);
            const dy = ellipse.xRadius * (ellipse.center[1] - coord[1]);
            const angle = Math.atan2(dy, dx);

            coord[0] = ellipse.center[0] - ellipse.xRadius * Math.cos(angle);
            coord[1] = ellipse.center[1] - ellipse.yRadius * Math.sin(angle);

            const scaledPoint = graphie.scalePoint(coord);
    ...
    addRotateHandle: (function() {
        const drawRotateHandle = function(graphie, center, radius, halfWidth,
                lengthAngle, angle, interacting) {
            const getRotateHandlePoint = function(offset,
            ...
            return graphie.raphael.path(
                ...
                stroke: null,
                fill: (interacting) ? KhanColors.INTERACTING
                                    : KhanColors.INTERACTIVE,
            });
        };
        ...
        return function(options) {
            ...
            const redrawRotateHandle = function(handleCoord) {
                ...
                const handleVec = kvector.subtract(handleCoord,
                        rotatePoint.coord);
            ...
            $(rotatePoint).on("move." + id, function() {
                ...
                rotateHandle.setCoord(kvector.add(
                    rotateHandle.coord,
                    delta
                ));
    ...
    addReflectButton: (function() {
        const drawButton = function(
            ...
            const leftCoord1 = kpoint.addVectors(buttonCoord, leftCenterVec, lineVec, leftVec);
            const leftCoord2 = kpoint.addVectors(buttonCoord, leftCenterVec, negLineVec, leftVec);
            const rightCoord1 = kpoint.addVectors(buttonCoord, rightCenterVec, lineVec, rightVec);
            const rightCoord2 = kpoint.addVectors(buttonCoord, rightCenterVec, negLineVec, rightVec);
...
function Protractor(graph, center) {
    ...
    const arrow = graph.raphael.path(
        " M" + arrowHelper(180, 6) +
        " L" + arrowHelper(180, 2) +
    ...
    this.makeTranslatable = function makeTranslatable() {
        ...
        $(setNodes).bind("vmousedown", function(event) {
            event.preventDefault();
            let startx = event.pageX - $(graph.raphael.canvas.parentNode).offset().left;
            let starty = event.pageY - $(graph.raphael.canvas.parentNode).offset().top;
...
function Ruler(graphie, options) {
    ...
    const mouseTarget = graphie.mouselayer.path(GraphUtils.svgPath([
    ...
    $(setNodes).bind("vmousedown", function(event) {
        event.preventDefault();
        let startx = event.pageX - $(graphie.raphael.canvas.parentNode).offset().left;
        let starty = event.pageY - $(graphie.raphael.canvas.parentNode).offset().top;
    ...
    const leftBottomHandle = graphie.addMovablePoint({
        ...
        onMove: function(x, y) {
            ...
            const center = kvector.scale(kvector.add([x, y], rightBottomHandle.coord), 0.5);
            const scaledCenter = graphie.scalePoint(center);
            const oldCenter = kvector.scale(kvector.add(leftBottomHandle.coord, rightBottomHandle.coord), 0.5);
    ...
    const rightBottomHandle = graphie.addMovablePoint({
        ...
        onMove: function(x, y) {
            ...
            const angle = Math.atan2(dy, dx) * 180 / Math.PI;
            const center = kvector.scale(kvector.add([x, y], leftBottomHandle.coord), 0.5);
            const scaledCenter = graphie.scalePoint(center);
            const oldCenter = kvector.scale(kvector.add(leftBottomHandle.coord, rightBottomHandle.coord), 0.5);
            const scaledOldCenter = graphie.scalePoint(oldCenter);
            const diff = kvector.subtract(scaledCenter, scaledOldCenter);
...
_.extend(MovableAngle.prototype, {
    ...
    _getClockwiseAngle: function(coords) {
        const clockwiseAngle = (GraphUtils.findAngle(
