...
$.tmpl = {
    ...
    attr: {
        "data-ensure": function(elem, ensure) {
            ...
            return function(elem) {
                ...
                // be chosen
                const result = !!(ensure && $.tmpl.getVAR(ensure));
                if (!result) {
        ...
        "data-else-if": function(elem, value) {
            ...
            const lastCond = $elem.data("lastCond");
        ...
        "data-each": function(elem, value) {
            ...
            if ((match = /^(.+) times(?: as (\w+))?$/.exec(value))) {
                const times = $.tmpl.getVAR(match[1]);
        ...
        "data-video-hint": function(elem) {
            let youtubeIds = $(elem).data("youtube-id");
            if (!youtubeIds) {
                return;
            }
            ...
            const author = $(elem).data("video-hint-author") || "Sal";
            const msg = i18n._("Watch %(author)s work through a very similar " +
                "problem:", {author: author});
            const preface = $("<p>").text(msg);

            const wrapper = $("<div>", { "class": "video-hint" });
            wrapper.append(preface);

            _.each(youtubeIds, function(youtubeId) {
                const href = "http://www.khanacademy.org/embed_video?v=" +
                            youtubeId;
        ...
        "var": function(elem, value) {
            ...
            if (name) {
                ...
                const setVAR = function(name, value) {
                    if (KhanUtil[name]) {
                        Khan.error("Defining variable '" + name + "' " +
                                   "overwrites utility property of same name.");
                    }
            ...
            // value
                ...
                if (value == null) {
                    ...
                    // with a "<" in Safari
                    const div = $("<div>");
                    const html = div.append(value + " ").html();
    ...
    getVAR: function(elem, ctx) {
        ...
        const code = elem.nodeName ? $(elem).text() : elem;
...
$.fn.tmpl = function() {
    ...
    function traverse(elem) {
        ...
        if (ret === false) {
            ...
            // appropriate places
            const origParent = elem.parentNode;
            const origNext = elem.nextSibling;
            ...
            $.each(ret.items, function(pos, value) {
                ...
                const conditionals = ["data-if", "data-else-if", "data-else"];

                let declarations = "";
                declarations += (ret.pos) ?
                    "var " + ret.pos + " = " + JSON.stringify(pos) + ";" :
                    "";
                declarations += (ret.value) ?
                    "var " + ret.value + " = " + JSON.stringify(value) + ";" :
                    "";

                for (let i = 0; i < conditionals.length; i++) {
                    const conditional = conditionals[i];
                    $(clone).find("[" + conditional + "]").each(function() {
                        let code = $(this).attr(conditional);
...
$.fn.extend({
    tmplApply: function(options) {
        ...
        const defaultApply = options.defaultApply || "replace";
        ...
        return this.each(function() {
            ...
            const hint = $this.data("apply") &&
                !$this.data("apply").indexOf("hint");
            ...
            if (name) {
                ...
                if (name in parent && !hint) {
                    ...
                    parent[name] = $.tmplApplyMethods[
                        $this.data("apply") || defaultApply]
...
$.extend({
    ...
    tmplApplyMethods: {
        ...
        appendVars: function(elem) {
            const parentEnsure = $(this).data("ensure") || "1";
            const childEnsure = $(elem).data("ensure") || "1";
            $(this).data("ensure",
