...
describe("Node accessibility", function() {
    ...
    it("should detect bad widgets", function() {
        assert.deepEqual(Perseus.accessibility.violatingWidgets(badItemData), ["interactive-graph"]);
    });

    it("should be fine with good items", function() {
        assert.deepEqual(Perseus.accessibility.violatingWidgets(goodItemData), []);
    });
});
