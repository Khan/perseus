 ...
 */
export function getAnswersFromWidgets(
 ...
 */
export function getPerseusAIData(perseusItem: PerseusItem): {
    answers: ReadonlyArray<string>;
    hints: ReadonlyArray<string>;
} {
    // Extract correct answers from the question widgets
    const answers = getAnswersFromWidgets(perseusItem.question.widgets);

    // Process each hint by injecting widget content
    const hints = perseusItem.hints.map((hint) =>
        injectWidgets(hint.content, hint.widgets),
    );
