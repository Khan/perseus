...
function buildNormalFunctionCallback(command: string, translated?: string) {
    return function (mathField: MathFieldInterface) {
        mathField.write(`\\${translated}\\left(\\right)`);
        mathField.keystroke("Left");
    };
}

const keyToMathquillMap: Record<Key, (translated: string) => MathFieldUpdaterCallback> = {
    EXP: () => handleExponent,
    ...
    SIN: (translated?: string) => {
        console.log(`\\${translated}\\left(\\right)`)
        return buildNormalFunctionCallback("sin", translated)
    },
