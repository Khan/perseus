...
describe("utils", () => {
    describe("multiplicationSymbol", () => {
        it("passes through convertDotToTimes in locales that don't override it", () => {
            const result1 = convertDotToTimesByLocale(true, "en");
            const result2 = convertDotToTimesByLocale(false, "en");
        ...
        it("overrides with false for locales that only use dot", () => {
            const result = convertDotToTimesByLocale(true, "az");

            expect(result).toBe(false);
        });

        it("overrides with true for locales that only use x", () => {
            const result = convertDotToTimesByLocale(false, "fr");
