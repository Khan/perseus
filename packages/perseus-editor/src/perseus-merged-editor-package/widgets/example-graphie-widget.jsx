...
class ExampleGraphieWidget extends React.Component<Props> {
    ...
    movePoint: (Coord) => void = (newCoord) => {
        this.change({
            coord: newCoord,
        });
    };

    _getGridConfig: ($FlowFixMe) => $FlowFixMe = (options) => {
    ...
    setupGraphie: ($FlowFixMe, $FlowFixMe) => $FlowFixMe = (
        graphie,
        options,
    ) => {
...
}: WidgetExports<typeof ExampleGraphieWidget>);
