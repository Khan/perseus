...
import SimpleButton from "./components/simple-button.jsx";
...
class ModeDropdown extends React.Component<ModeDropdownProps> {
    _handleSelectMode = (event: SyntheticInputEvent<HTMLSelectElement>) => {
        if (this.props.onChange) {
            ...
            const value = ((event.target.value: any): EditorMode);
...
const Header = ({depth, ...props}: HeaderProps) => {
...
const NodeContainer = (props: {|
    ...NodePropTypes<>,
    controls?: $ReadOnlyArray<React.Node>,
    name?: string,
|}) => {
    const {
        shape,
...
|};
const ArrayContainer = (props: ArrayContainerProps) => {
...
type WithStickiness<T> = {|
    ...$Exact<T>,
    ...WithStickinessProps,
|};

type WithoutStickiness<T> = $Exact<$Rest<T, WithStickinessProps>>;
...
const withStickiness = <
    Config: {...},
    Component: React.ComponentType<WithStickiness<Config>>,
>(
    WrappedComponent: Component,
): React.ComponentType<WithoutStickiness<React.ElementConfig<Component>>> => {
    type State = {|sticky: boolean|};
    return class StickyComponent extends React.Component<Config, State> {
        stickynessTimer: IntervalID;
        ...
        updateStickiness = () => {
            ...
            // $FlowFixMe[incompatible-use]: findDOMNode returns null | Text | Element
...
const ItemNodeContent = withStickiness(
    (props: {|
        ...NodePropTypes<ContentShape, ContentNode>,
        ...WithStickinessProps,
    |}) => {
...
class MultiRendererEditor extends React.Component<MultiRendererEditorProps> {
    layout: ?React.ElementRef<$FlowFixMe>;

    _renderLayout: () => React.Node = () => {
    ...
    _renderPreview: () => React.Element<"div"> = () => {
        return (
            <div>
                <ModeDropdown
                    currentMode={this.props.editorMode}
    ...
    getSerializedState: () => any | void = () => {
        if (this.layout) {
            return this.layout.getSerializedState();
        }
    };

    restoreSerializedState: (state: any) => void = (state: $FlowFixMe) => {
        if (this.layout) {
            this.layout.restoreSerializedState(state);
        }
    };

    _renderContent: () => React.Element<"div"> | React.Node = () => {
