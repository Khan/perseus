...
type StepControlButtonProps = $FlowFixMe;
...
type SequenceEditorProps = $FlowFixMe;
...
class SequenceEditor extends React.Component<SequenceEditorProps> {
    ...
    _handleEditorChange: (number, $FlowFixMe) => void = (i, newProps) => {
    ...
    _handleMoveStepEarlier: (number) => void = (i) => {
        if (i === 0) {
            return;
        }
        const steps = _.clone(this.props.json);
    ...
    _handleMoveStepLater: (number) => void = (i) => {
        const steps = _.clone(this.props.json);
        if (i + 1 === steps.length) {
            return;
        }
    ...
    _handleAddStepAfter: (number) => void = (i) => {
    ...
    _handleRemoveStep: (number) => void = (i) => {
        const steps = _.clone(this.props.json);
        steps.splice(i, 1);
        this.props.onChange({
            json: steps,
