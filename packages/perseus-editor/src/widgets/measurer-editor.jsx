...
class MeasurerEditor extends React.Component<Props> {
    ...
    _changeUrl: (SyntheticInputEvent<>) => void = (e) => {
        this._changeImage("url", e.target.value);
    };

    _changeTop: ($FlowFixMe) => void = (newTop) => {
        this._changeImage("top", newTop);
    };

    _changeLeft: ($FlowFixMe) => void = (newLeft) => {
        this._changeImage("left", newLeft);
    };

    _changeImage: (string, $FlowFixMe) => void = (subProp, newValue) => {
    ...
    ) => $ReadOnlyArray<React.Element<"option">> = (choices) => {
