import {jsonStreamParser, number, Tokenizer} from "./json-stream-parser";
...
describe("number", () => {
    it("accepts valid numbers", () => {
        ...
        for (const value of valid) {
            expect(value.match(number)?.[0]).toEqual(value);
            expect(() => JSON.parse(value)).not.toThrowError();
        }
    })
    ...
    it("rejects invalid numbers", () => {
        ...
        for (const value of invalid) {
            expect(value.match(number)?.[0]).not.toEqual(value);
            expect(() => JSON.parse(value)).toThrowError();
        }
    })
})
