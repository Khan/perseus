...
### PR 1 — `linear-system`
**Also:** In `knip.config.ts`, add a `packages/perseus-score` workspace entry (extending `basePackageConfig`) with `ignore: ["src/widgets/interactive-graph/sub-scorers/"]` so knip doesn't flag sub-scorers as dead code until PR 15.  
