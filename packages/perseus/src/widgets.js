...
type Editor = $FlowFixMe;

const widgets: {|[string]: WidgetExports<>|} = {};
const editors = {};
...
export const getWidget = (name: string): ?React.ComponentType<any> => {
...
export const getEditor = (name: string): ?Editor => {
    return _.has(editors, name) ? editors[name] : null;
};

export const getTransform = (name: string): ?WidgetTransform => {
    return _.has(widgets, name) ? widgets[name].transform || _.identity : null;
};

export const getVersion = (name: string): ?Version => {
...
export const getVersionVector = (): {|[string]: Version|} => {
    const version = {};
 ...
 */
export const getStaticTransform = (type: string): ?WidgetTransform => {
    const widgetInfo = widgets[type];
