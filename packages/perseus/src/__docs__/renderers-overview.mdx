...
For more details on specific renderer implementations and usage, refer to their individual documentation pages and the API reference.
...
**2. `ServerItemRenderer` renders two main components:** - The question content using `Renderer` (with the item's question content and widgets) - Hints content using `HintsRenderer` (which uses `HintRenderer` for each hint)

```
PerseusItem → ServerItemRenderer
  → Renderer (question content)
  → HintsRenderer (hints content)
    → HintRenderer (for each hint)
```
...
```
PerseusArticle → ArticleRenderer
...
The `ServerItemRenderer` coordinates scoring of exercises through a well-defined process:

**1. Widget Score Collection**

-   The underlying `Renderer` component's `score()` method collects scores from all widgets
-   It uses `scoreWidgetsFunctional` from the `@khanacademy/perseus-score` package
-   Each widget provides its own scoring logic through its implementation

**2. User Input Management**

-   The renderer maintains the current state of all widgets through:
    -   `getUserInputMap()` - Gets the current input state from all widgets
    -   `setInputValue()` - Sets input values programmatically
-   This state is used both for scoring and for restoring saved work

**3. Score Aggregation**

-   Individual widget scores are combined using the `flattenScores` function
-   This produces a unified `PerseusScore` object with:
    -   `type` - The type of score (e.g., "points", "invalid")
    -   `earned` - Points earned by the user
    -   `total` - Total possible points
    -   `message` - Optional feedback message
    -   `widgets` - Detailed per-widget scores

**4. Score Feedback**

-   When a score is provided through props, `ServerItemRenderer` updates its state:
    -   Sets `questionCompleted` based on the `correct` property
    -   This triggers visual feedback for users (like highlighting correct/incorrect answers)
-   Widgets may show explanations or rationales when scoring is complete

**5. Score Reporting**

-   The parent component can access the score through renderer methods
-   The scoring state can be serialized for saving user progress
-   Articles with `graded-group` widgets handle scoring internally
