...
import MovableHelperMethods from "./movable-helper-methods.js";

/**
 * Compute the correct vendor-prefixed `transform`.
 */
let prefixedTransform = null;
function computePrefixedTransform(): ?string {
...
const InteractiveUtil = {
     ...
     */
    createGettersFor: function (Class: $FlowFixMe, defaults: {...}) {
     ...
     */
    addMovableHelperMethodsTo: function (Class: $FlowFixMe) {
    ...
    arrayify: function (
        funcOrArray:
            | ((...args: $ReadOnlyArray<mixed>) => mixed)
            | $ReadOnlyArray<mixed>,
    ): $ReadOnlyArray<mixed> {
        if (funcOrArray == null) {
            return [];
     ...
     */
    getPrefixedTransform: function (): ?string {
