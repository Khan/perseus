...
import type {Widget, PerseusScore, KEScore} from "./types.js";
 ...
 */
function firstNumericalParse(text: string): ?ParsedValue {
 ...
 */
function gridStepFromTickStep(tickStep: number, scale: number): ?number {
 ...
 *      }
 */
const DeprecationMixin: $FlowFixMe = {
 ...
 */
function extractPointerLocation(event: $FlowFixMe): ?Position {
    let touchOrEvent;
...
function getImageSize(url: string, callback: (number, number) => void): void {
    const img = new Image();
    img.onload = function () {
