...
class Plotter extends React.Component<$FlowFixMe, $FlowFixMe> {
    shouldSetupGraphie: boolean;
    _isMounted: boolean = false;
    horizHairline: $FlowFixMe;
    hairlineRange: $FlowFixMe;
    graphie: $FlowFixMe;
    ...
    componentDidUpdate(prevProps: $FlowFixMe, prevState: $FlowFixMe) {
    ...
    setupGraphie: ($FlowFixMe) => void = (prevState) => {
        const self = this;
    ...
    showHairlines: ($FlowFixMe) => void = (point) => {
    ...
    labelCategory: ($FlowFixMe, $FlowFixMe) => Promise<$FlowFixMe> = (
        x,
        category,
    ) => {
        const isMobile = this.props.apiOptions.isMobile;
    ...
    setupCategories: ($FlowFixMe) => void = (config) => {
        const self = this;
        const c = config;
    ...
    _clampValue: (number, number, number) => number = (v, min, max) => {
    ...
    setupBar: ($FlowFixMe) => number = (args) => {
        const isMobile = this.props.apiOptions.isMobile;
     ...
     */
    setupLine: (number, number, $FlowFixMe) => number = (
        i,
        startHeight,
        config,
    ) => {
    ...
    setupDotplot: ($FlowFixMe, $FlowFixMe) => $FlowFixMe = (i, config) => {
        const graphie = this.graphie;
        const isMobile = this.props.apiOptions.isMobile;
    ...
    setupPic: ($FlowFixMe, $FlowFixMe) => $FlowFixMe = (i, config) => {
        const graphie = this.graphie;
    ...
    setupTiledPlot: (
        ...
        $FlowFixMe,
        $FlowFixMe,
    ) => $FlowFixMe = (i, bottomMargin, config, createImage) => {
        const self = this;
        const c = config;
        const graphie = self.graphie;
    ...
    setPicHeight: (number, number) => void = (i, y) => {
    ...
    changeAndTrack: ($FlowFixMe) => void = (data) => {
        this.props.onChange(data);
        this.props.trackInteraction();
    };

    drawPicHeights: ($FlowFixMe, $FlowFixMe) => void = (values, prevValues) => {
        const self = this;
        const graphie = self.graphie;
        const pics = graphie.pics;
