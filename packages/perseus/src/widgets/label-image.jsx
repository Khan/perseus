...
} from "./label-image/types.js";

type MarkersState = {|markers: $ReadOnlyArray<InteractiveMarkerType>|};
...
class LabelImage extends React.Component<LabelImageProps, LabelImageState> {
    ...
    dismissMarkerPopup() {
        ...
        this.setState({selectedMarkerIndex: -1}, () => {
            ...
            if (marker) {
                ((ReactDOM.findDOMNode(marker): any): HTMLElement).focus();
            }
        });
    }
    ...
    handleDocumentClick: (e: MouseEvent) => void = (e: MouseEvent) => {
        ...
        if (this._selectedMarkerPopup && this._answerChoices) {
            ...
            const targetElement = ((e.target: any): Node);
            ...
            // contained within.
            const containsEventTarget = (element, depth = 3) =>
                element &&
    ...
    renderMarkers(): $ReadOnlyArray<React.Node> {
        ...
        return markers.map((marker, index): React.Node => {
            const score = LabelImage.gradeMarker(marker);
...
export default ({
    ...
    accessible: true,
    isLintable: true,
}: WidgetExports<typeof LabelImage>);
