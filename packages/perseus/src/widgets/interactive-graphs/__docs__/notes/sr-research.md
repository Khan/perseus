...
1. **Expected behavior of the screen reader** — see [Goals & Requirements](#goals--requirements). The "Designer's planned updates" and "UXR-derived requirements" subsections describe the target behavior; "Level Access audit findings" and "None-type / static / locked-figure requirements" describe the bugs we expect to close.
2. **Technical notes to consider during implementation** — see [Related System Architecture](#related-system-architecture) for what must be touched, and [Open Questions](#open-questions) for the decisions implementers need before they start.
...
The questions below need product / design / a11y sign-off before implementation. Each lists options with the case for and against.
...
- **OQ1** → A. Trap focus on all graphs (bounded + unlimited).
- **OQ2a** → B. Two strings (bounded vs. unlimited).
- **OQ2b** → A. Keep Ctrl + Shift + Arrow; warn users in the instructions to toggle off Virtual Cursor / Browse mode.
- **OQ2c** → A. Implement Insert + I on Windows and Fn + Enter + I on Mac (platform-detected dual binding); same mode-toggle warning in the instructions covers both. Confirm final instruction copy with Darrell during implementation.
- **OQ3** → C. Hybrid: central pure-function handler for universal events; `useGraphAnnouncer` escape hatch for graph-specific math-heavy copy. Routing rule and initial event-by-event assignments live in OQ3.
- **OQ4** → A. Reorder DOM so instructions come first.
- **OQ5** → A. Add optional `pointLabels?: string[]` on point/polygon graph types; fall back to "Point N" per-index when unset.
- **OQ6a** → A primary (`role="figure"`), B fallback (`role="application"`) if cross-SR testing flags issues.
