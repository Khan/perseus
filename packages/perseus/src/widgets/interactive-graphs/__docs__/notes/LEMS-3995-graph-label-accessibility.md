...
- [ ] In `start-coords-settings.tsx`, pass `pointLabels` and `onChangePointLabels` from `props` to the new graph type's child component (one-line addition in the switch). **Note:** access `pointLabels` inline as `props.pointLabels` inside the narrowed `case` branch (the `type === "<graph-type>"` discriminant already provides the narrowing). Don't destructure `pointLabels` at the top of the function — `PerseusGraphTypeNone` (one variant of the `PerseusGraphType` union that `Props` extends) intentionally does NOT declare the field, so destructuring it from the un-narrowed union fails with TS2339. A `"pointLabels" in props` guard would also narrow correctly if needed, but the inline access inside the existing `case` block is simpler.
...
- **Convert `type Props = ...` declarations to `interface` across the start-coords editor files.** Team convention prefers `interface` over `type` for object props. In PR 2, `start-coords-settings.tsx`'s `Props` stays as `type` because it intersects the `PerseusGraphType` discriminated union (`type Props = PerseusGraphType & {...}`) — interfaces cannot extend or compose with unions. The compatible pattern is to split into `interface ExtraProps {...}` for the added fields plus `type Props = PerseusGraphType & ExtraProps` for the intersection. Apply across every `start-coords-*.tsx` file that uses the same pattern (`start-coords-{line,multiline,angle,sinusoid,quadratic,exponential,logarithm,tangent,circle,absolute-value,point}.tsx`) so all graph types adopt the convention in one focused refactor. **Timing:** Ivy plans to do this immediately after PR 2 lands so that PRs 3–7 inherit the new pattern instead of each touching a `type Props` declaration that gets converted later — avoids per-PR conflict churn.
...
| **Take-2:** In `start-coords-settings.tsx`, access `props.pointLabels` inline inside the narrowed `case "point":` branch (do NOT destructure `pointLabels` at the top, and keep `PerseusGraphTypeNone` as `{type: "none"}` — no `pointLabels` field) | Reviewer asked *"Might as well expose the pointLabels since you also expose onChangePointLabels"* — declined for a typing reason: `pointLabels` lives on the `PerseusGraphType` discriminated union, and `PerseusGraphTypeNone` intentionally lacks the field, so destructuring from the un-narrowed `Props` fails with TS2339. Tried adding `pointLabels?: undefined` to `PerseusGraphTypeNone` so the destructure would compile, but that broke two tstyche type-tests (`interactive-graph-widget.typetest.ts`, `interactive-graph-user-input.typetest.ts`) which assert the parser's return type is structurally identical to the schema — the parser at `parsePerseusGraphTypeNone` doesn't (and shouldn't) parse `pointLabels`, so schema and parser diverged. Reverted the schema change. The chosen pattern accesses `props.pointLabels` inline inside the `case "point":` branch where the `type === "point"` discriminant already narrows the union to a variant that declares the field. PRs 3–7 should follow the same pattern as they widen the gate. |
