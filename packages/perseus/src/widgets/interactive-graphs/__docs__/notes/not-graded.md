...
What's missing is the authoring experience (a toggle in the editor), the learner experience (a visual "not graded" indicator), and the review-mode behavior (suppress the correct-answer reveal when `graded: false`). Because other widgets may eventually benefit from this feature, the UI plumbing should live in shared code (`WidgetEditor`, `WidgetContainer`, `Renderer`) rather than inside the interactive graph widget directly.
...
3. **Learner indicator** — When `graded: false` is rendered for a learner, a visible label makes it clear the widget won't be scored.
...
| "Not graded" visual indicator | ❌ Missing | `widget-container.tsx` has no graded-aware rendering |
...
### 6. Add "Not graded" indicator in `WidgetContainer`

In `packages/perseus/src/widget-container.tsx`, when `props.graded === false`, render a visible label (e.g., "This graph is for your use only and will not be graded.") near the widget. Placing this in `WidgetContainer` rather than inside the IG widget makes it available to any future widget that gains the toggle.
...
- **`widget-container.tsx`**: Test that the "not graded" message renders when `graded={false}` and not otherwise.
...
| `packages/perseus/src/widget-container.tsx` | Render "not graded" message when `graded === false` |
...
| `packages/perseus/src/widgets/interactive-graphs/interactive-graph.tsx` | Forward `graded` to `StatefulMafsGraph` |
