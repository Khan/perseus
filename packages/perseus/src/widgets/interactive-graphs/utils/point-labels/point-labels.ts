...
// `showPointLabels` field. The same per-index label string drives both the
// visible on-canvas <TextLabel> and the screen-reader aria-label, so the
// two never disagree (WCAG 2.5.3 "Label in Name").
//
// `showPointLabels: true` requires the author to provide `pointLabels`. We do
// NOT auto-generate fallback letters (A, B, C, …) here. Authoring-time
...
// sync. When `showPointLabels` is off, returns `pointLabels` unchanged
// (legacy behaviour). When on, returns one entry per point — missing or
// empty entries become `""`.
