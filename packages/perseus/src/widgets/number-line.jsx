...
const _label = (graphie, labelStyle, pos, value, base: number): $FlowFixMe => {
    value = value || pos;
...
// $FlowFixMe[prop-missing]
const TickMarks: $FlowFixMe = Graphie.createSimpleClass((graphie, props) => {
    // Avoid infinite loop
...
type Props = {|
    ...
    onFocus: ($FlowFixMe) => void,
    onBlur: ($FlowFixMe) => void,
    onChange: ($FlowFixMe, ?() => void) => void,
...
class NumberLine extends React.Component<Props, State> {
    ...
    onNumDivisionsChange: (number, $FlowFixMe) => void = (numDivisions, cb) => {
    ...
    snapNumLinePosition: ($FlowFixMe, number) => number = (
        props,
        numLinePosition,
    ) => {
    ...
    // position
    movePosition: (number) => void = (targetPosition) => {
        this.change({numLinePosition: targetPosition});
        this.props.trackInteraction();
    };

    _renderNumberLinePoint: ($FlowFixMe) => React.Node = (props) => {
    ...
    _getInequalityEndpoint: ($FlowFixMe) => [number, number] = (props) => {
    ...
    _renderInequality: ($FlowFixMe) => React.Node = (props) => {
...
const numberLineTransform: ($FlowFixMe) => $FlowFixMe = (editorProps) => {
...
}: WidgetExports<typeof NumberLine>);
