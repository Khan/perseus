...
Instead of one editable content string with embedded blank markers, **content is an ordered list of typed *segments***: `text`, `tex`, `image`, and `blank`. This is a *lite, purpose-built* WYSIWYG — not a general rich-text editor.
...
| **TeX escaping is gibberish** — a blank inside math forces authors to terminate TeX on both sides: `$12+$ {{1}} $=16$`. | A blank is a **sibling segment** between two TeX segments. It is never inside a TeX string, so there is nothing to escape or terminate. |
...
export type FITBTextSegment  = {type: "text";  id: string; text: string};   // plain-text run (may be a whole sentence)
export type FITBTexSegment   = {type: "tex";   id: string; tex: string};     // raw TeX; rendered in preview, no escaping
...
export type FITBBlankSegment = {
    ...
    correct: string;                                   // choiceId
    ariaLabel?: string;                                // custom SR label; defaults to "Blank N" (by doc order)
};

export type FITBChoiceSegment = FITBTextSegment | FITBTexSegment | FITBImageSegment;  // choices zone (no Blank)
...
- **Text field is `text`, not `content`** — avoids `options.content[i].content`, and is symmetric with `tex` segments (`options.content[i].text` / `.tex`).
- **Blank ids are safe here.** Generated on segment creation, carried on the object, never hand-typed — this is the whole point of the pivot.
- **`correct` references a choice by id** (`choices[].id`); the action-menu Correct dropdown lists the current choices. Deleting a referenced choice clears/prunes that reference (label-image already does this for markers).
...
- **Shared `Card Types` palette** — Plain Text / TeX / Image / Blank; places a segment of that type into a zone.
...
- **Per-segment editing** — Text/TeX segments are edited **directly in place** (typing into a narrow action-menu tooltip would be too cramped); Blank and Image segments use the **action menu** for their settings (Blank: Correct dropdown, Display Type, Alt Text, Remove; Image: URL, Description, Alt Text, Remove).
...
**There is essentially no parsing.** Content is already structured JSON. Render is a `.map` over `content`: `text`→inline text, `tex`→rendered TeX, `image`→`<img>`, `blank`→the `Blank` component. No marker regex, no markdown-rule change, no TeX-escape collisions.
...
- **i18n extraction.** Perseus generally extracts translatable strings from a markdown content string. FITB's translatable strings live in structured fields (`text.text`, `image.alt`/`longDescription`, `choice.*`, `blank.ariaLabel`). Nested-translatable options have precedent (radio choices, label-image alt), but the extraction pipeline must reach these fields. **Verify before productionizing; irrelevant to the prototype.**
...
export function getFillInTheBlankPublicWidgetOptions(
    ...
    return {
        ...
        content: options.content.map((seg) =>
            seg.type === "blank" ? getBlankPublicData(seg) : seg,  // text/tex/image have no secrets → pass through
        ),
...
**Whitespace convention.** Since the student flows adjacent segments together, inter-segment spacing lives **inside the text segments** (a text segment is `"The "` with a trailing space; a period segment is `". "` with no leading space). The renderer just concatenates. See the worked example in [Appendix A](#appendix-a--worked-example).
...
| Blank action menu | interactive-graph locked-figure settings + WB `Popover`/`Dropdown` | Correct (choice dropdown) · Display Type (segmented/select) · Alt Text (field, with Auto-generate) · Remove. |
...
| Text / TeX segment editors | `form-wrapped-text-field` / inline textarea | Edited in place; TeX stores raw TeX, rendered in preview. See §7 for overflow. |
...
Sarah's sample question: *"My name is Spongebob and I live in a `[Pineapple]` under the `[Sea]`. `[🍍house image]`"* — three text segments, three blanks, and a choice bank of two text choices + one image (Spongebob's pineapple house). Blank 1 → `Pineapple`, Blank 2 → `Sea`, Blank 3 → the house image.
...
```json
{
  ...
  "widgets": {
    "fill-in-the-blank 1": {
      ...
      "options": {
        "content": [
          { "type": "text",  "id": "seg-1",   "text": "My name is Spongebob and I live in a " },
          { "type": "blank", "id": "blank-1", "displayType": "normal", "correct": "choice-pineapple" },
          { "type": "text",  "id": "seg-2",   "text": " under the " },
          { "type": "blank", "id": "blank-2", "displayType": "normal", "correct": "choice-sea" },
          { "type": "text",  "id": "seg-3",   "text": ". " },
          { "type": "blank", "id": "blank-3", "displayType": "normal", "correct": "choice-house" }
        ],
        "choices": [
          { "type": "text",  "id": "choice-pineapple", "text": "Pineapple" },
          { "type": "text",  "id": "choice-sea",       "text": "Sea" },
