...
const PERSEUS_COMPOSE = path.join(
    FRONTEND_ROOT,
    "genfiles/devserver/docker-compose.perseus.yml",
);
...
// ---------------------------------------------------------------------------
// Generate docker-compose.perseus.yml
// ---------------------------------------------------------------------------

function generatePerseusCompose(): void {
    fs.mkdirSync(path.dirname(PERSEUS_COMPOSE), {recursive: true});
    // Mount all three directories at their exact host paths so the absolute
    // paths stored in pnpm-lock.yaml (file: specifiers) and pnpm's virtual
    // store symlinks resolve correctly inside the container.
    //
    // The base docker-compose.yml mounts FRONTEND_ROOT at /webapp/services/static/
    // (the container's working directory), but pnpm lock entries use absolute host
    // paths like /Users/.../frontend/genfiles/perseus-local-dev-tmp/... so we also
    // mount the frontend at its original host path.
    //
    // PERSEUS_TMP_DIR lives inside FRONTEND_ROOT/genfiles/ so it is covered by the
    // FRONTEND_ROOT mount below — no separate entry is needed for it.
    fs.writeFileSync(
        PERSEUS_COMPOSE,
        [
            "# Generated by utils/start-perseus-in-local-server.ts — do not edit manually.",
            "# Mounts Perseus and the frontend repo at their exact host paths so",
            "# all absolute paths in pnpm-lock.yaml and virtual-store symlinks",
...
function patchContainerTs(): void {
    // Insertion 1: fs import (containers.ts doesn't import fs by default)
    patchFile(
        CONTAINERS_TS,
        `import path from "node:path";`,
        `import fs from "node:fs";`,
    );
    // Insertion 2: variable declaration after workingOnCompose declaration
    patchFile(
        CONTAINERS_TS,
        `    const workingOnCompose = path.join(\n        GENFILES_DEVSERVER_DIR,\n        "docker-compose.working-on.yml",\n    );`,
        `    const perseusCompose = path.join(GENFILES_DEVSERVER_DIR, "docker-compose.perseus.yml");`,
    );
    // Insertion 3: splice into args array between workingOnCompose and "-p"
    patchFile(
        CONTAINERS_TS,
        `            "-f",\n            workingOnCompose,`,
        `            ...(fs.existsSync(perseusCompose) ? ["-f", perseusCompose] : []),`,
    );
}
...
function cleanup(): void {
    ...
    // 4. Remove Perseus compose override
    if (fs.existsSync(PERSEUS_COMPOSE)) {
        fs.rmSync(PERSEUS_COMPOSE);
    }
...
// Step 3: Generate docker-compose.perseus.yml
